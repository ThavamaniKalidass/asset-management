...
router.get('/desks', async (_req, res) => {
    try {
        ...
        res.json(result.rows.map((r) => r.desk_number));
    }
    catch (err) {
...
router.get('/desk/:deskNumber', async (req, res) => {
    try {
        ...
        const result = await query('SELECT id, asset_type, brand, model_number, serial_number, desk_number, created_at, updated_at FROM assets WHERE LOWER(desk_number) = LOWER($1) ORDER BY asset_type', [deskNumber]);
        res.json({
...
router.get('/recent', async (req, res) => {
    try {
        ...
        const result = await query('SELECT id, asset_type, brand, model_number, serial_number, desk_number, created_at FROM assets ORDER BY created_at DESC LIMIT $1', [limit]);
...
router.get('/:id', async (req, res) => {
    try {
        const result = await query('SELECT id, asset_type, brand, model_number, serial_number, desk_number, created_at, updated_at FROM assets WHERE id = $1', [req.params.id]);
...
router.post('/', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await query(`INSERT INTO assets (asset_type, brand, model_number, serial_number, desk_number)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, asset_type, brand, model_number, serial_number, desk_number, created_at, updated_at`, [asset_type, brand, model_number, serial_number, desk_number]);
    ...
    try {
        ...
        for (const asset of assets) {
            ...
            try {
                const result = await query(`INSERT INTO assets (asset_type, brand, model_number, serial_number, desk_number)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING id, asset_type, brand, model_number, serial_number, desk_number`, [asset_type, brand, model_number, serial_number, desk_number]);
...
router.get('/qr/:deskNumber', async (req, res) => {
    try {
        const { deskNumber } = req.params;
        ...
        const assetsResult = await query('SELECT id, asset_type, brand, model_number, serial_number, desk_number FROM assets WHERE LOWER(desk_number) = LOWER($1) ORDER BY asset_type', [deskNumber]);
        res.json({
            desk_number: deskNumber,
